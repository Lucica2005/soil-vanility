...
def preprocess_wosis():
    """Preprocess WOSIS data for TabDiff"""
    
    with open(f'{INFO_PATH}/wosis.json', 'r') as f:
        info = json.load(f)
    
    # Load the preprocessed data
    train_df = pd.read_csv('data/wosis/wosis.csv')
    test_df = pd.read_csv('data/wosis/wosis_test.csv')
    
    print(f"WOSIS train shape: {train_df.shape}")
    print(f"WOSIS test shape: {test_df.shape}")
    
    # No validation set for WOSIS, create empty one
    val_df = test_df.iloc[:0].copy()  # Empty dataframe with same structure
    
    # Process the data according to TabDiff requirements
    process_data_generic('wosis', train_df, test_df, val_df, info, has_val=False)

def preprocess_wosis_dcr():
    """Preprocess WOSIS data for DCR evaluation"""
    
    with open(f'{INFO_PATH}/wosis_dcr.json', 'r') as f:
        info = json.load(f)
    
    # Load the preprocessed data
    train_df = pd.read_csv('data/wosis_dcr/wosis_dcr.csv')
    test_df = pd.read_csv('data/wosis_dcr/wosis_dcr_test.csv')
    
    print(f"WOSIS DCR train shape: {train_df.shape}")
    print(f"WOSIS DCR test shape: {test_df.shape}")
    
    # No validation set for WOSIS DCR, create empty one
    val_df = test_df.iloc[:0].copy()  # Empty dataframe with same structure
    
    # Process the data according to TabDiff requirements
    process_data_generic('wosis_dcr', train_df, test_df, val_df, info, has_val=False)

def preprocess_wosis_large(name):
    """Preprocess WOSIS large data (all depths)"""
    
    with open(f'{INFO_PATH}/{name}.json', 'r') as f:
        info = json.load(f)
    
    # Load the preprocessed data
    train_df = pd.read_csv(f'data/{name}/{name}.csv')
    test_df = pd.read_csv(f'data/{name}/{name}_test.csv')
    
    print(f"WOSIS large train shape: {train_df.shape}")
    print(f"WOSIS large test shape: {test_df.shape}")
    
    # No validation set for WOSIS large, create empty one
    val_df = test_df.iloc[:0].copy()  # Empty dataframe with same structure
    
    # Process the data according to TabDiff requirements
    process_data_generic(name, train_df, test_df, val_df, info, has_val=False)

def process_data_generic(name, train_df, test_df, val_df, info, has_val=True):
    """Generic data processing function for TabDiff"""
    ...
    # Handle missing values (WOSIS data should be clean already)
    ...
    X_cat_train = train_df[cat_columns].to_numpy() if cat_columns else np.empty((len(train_df), 0))
    y_train = train_df[target_columns].to_numpy()

    X_num_test = test_df[num_columns].to_numpy().astype(np.float32)
    X_cat_test = test_df[cat_columns].to_numpy() if cat_columns else np.empty((len(test_df), 0))
    y_test = test_df[target_columns].to_numpy()

    if has_val:
        X_num_val = val_df[num_columns].to_numpy().astype(np.float32)
        X_cat_val = val_df[cat_columns].to_numpy() if cat_columns else np.empty((len(val_df), 0))
        y_val = val_df[target_columns].to_numpy()
    else:
        # Create empty validation arrays
        X_num_val = np.empty((0, len(num_columns)), dtype=np.float32)
        X_cat_val = np.empty((0, len(cat_columns)), dtype=object) if cat_columns else np.empty((0, 0))
        y_val = np.empty((0, len(target_columns)))
    ...
    # Update info with data counts
    info['train_num'] = len(train_df)
    info['test_num'] = len(test_df)
    info['val_num'] = len(val_df) if has_val else 0
    
    # Detect integer columns among numerical columns
    complete_df = pd.concat([train_df, test_df, val_df] if has_val else [train_df, test_df], axis=0)
    int_columns = []
    int_col_idx = []
    int_col_idx_wrt_num = []
    ...
    # Create metadata for TabDiff
    metadata = {}
    metadata['columns'] = {}
    
    # Add numerical columns to metadata
    for i in num_col_idx:
        metadata['columns'][i] = {}
        metadata['columns'][i]['sdtype'] = 'numerical'
    
    # Add categorical columns to metadata
    for i in cat_col_idx:
        metadata['columns'][i] = {}
        metadata['columns'][i]['sdtype'] = 'categorical'
    
    # Add target columns to metadata
    ...
    print(f'Train: {info["train_num"]}, Val: {info["val_num"]}, Test: {info["test_num"]}')
    print(f'Numerical: {len(num_col_idx)}, Categorical: {len(cat_col_idx)}, Target: {len(target_col_idx)}')
...
def process_data(name):
    ...
    if name == 'news':
        ...
        preprocess_wosis()
        return  # Early return since we handle everything in preprocess_wosis
    elif name == 'wosis_dcr':
        preprocess_wosis_dcr()
        return  # Early return since we handle everything in preprocess_wosis_dcr
    elif 'WOSIS' in name:
        preprocess_wosis_large(name)
        return  # Early return since we handle everything in preprocess_wosis_large
