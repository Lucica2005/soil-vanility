...
class Trainer:
    def __init__(
        ...
        # Handle DataParallel models for EMA
        if isinstance(self.diffusion, torch.nn.DataParallel):
            self.ema_model = deepcopy(self.diffusion.module._denoise_fn)
            print("Multi-GPU mode: EMA model created from DataParallel module")
        else:
        ...
        # Handle noise schedules for DataParallel
        if isinstance(self.diffusion, torch.nn.DataParallel):
            self.ema_num_schedule = deepcopy(self.diffusion.module.num_schedule)
            self.ema_cat_schedule = deepcopy(self.diffusion.module.cat_schedule)
        else:
        ...
        if self.ckpt_path is not None:
            ...
            if isinstance(self.diffusion, torch.nn.DataParallel):
                self.diffusion.module._denoise_fn.load_state_dict(state_dicts['denoise_fn'])
                self.diffusion.module.num_schedule.load_state_dict(state_dicts['num_schedule'])
                self.diffusion.module.cat_schedule.load_state_dict(state_dicts['cat_schedule'])
            else:
    ...
    def _run_step(self, x, closs_weight, dloss_weight):
        ...
        dloss, closs, ec_loss = self.diffusion.mixed_loss(x)

        loss = dloss_weight * dloss + closs_weight * closs
        if self.diffusion.enable_ec_validation:
            loss += self.diffusion.ec_weight * ec_loss
        loss.backward()
        self.optimizer.step()

        return dloss, closs, ec_loss
    ...
    def compute_loss(self):      # eval loss is not weighted
        ...
        for batch in data_iter:
            ...
            self.diffusion.eval()
            with torch.no_grad():
                batch_dloss, batch_closs, batch_ec_loss = self.diffusion.mixed_loss(x)
            curr_dloss += batch_dloss.item() * len(x)
            curr_closs += batch_closs.item() * len(x)
            curr_ec_loss += batch_ec_loss.item() * len(x) if self.diffusion.enable_ec_validation else 0.0
            curr_count += len(x)
        mloss = np.around(curr_dloss / curr_count, 4)
        gloss = np.around(curr_closs / curr_count, 4)
        ec_loss = np.around(curr_ec_loss / curr_count, 4) if self.diffusion.enable_ec_validation else 0.0
        return mloss, gloss, ec_loss
    ...
    def run_loop(self):
        ...
        if self.diffusion.enable_ec_validation:
            print("EC validation is enabled............")
        ...
        for epoch in range (start_epoch, self.steps):
            ...
            for batch in pbar:
                x = batch.float().to(self.device)
                batch_dloss, batch_closs, batch_ec_loss = self._run_step(x, closs_weight, dloss_weight)
                curr_dloss += batch_dloss.item() * len(x)
                curr_closs += batch_closs.item() * len(x)
                curr_ec_loss += batch_ec_loss.item() * len(x) if self.diffusion.enable_ec_validation else 0.0
                curr_count += len(x)
                # Calculate total loss including EC loss if enabled
                total_loss_display = (curr_dloss + curr_closs) / curr_count
                if self.diffusion.enable_ec_validation:
                    total_loss_display += self.diffusion.ec_weight * (curr_ec_loss / curr_count)
                ...
                # Add EC loss if enabled
                if self.diffusion.enable_ec_validation:
                    postfix_dict["ECLoss"] = np.around(curr_ec_loss/curr_count, 4)
                
                # Add total loss and weights
                postfix_dict.update({
                    "TotalLoss": np.around(total_loss_display, 4),
            ...
            if self.diffusion.enable_ec_validation:
                ec_loss_avg = np.around(curr_ec_loss / curr_count, 4)
                loss_dict["loss/ec_loss"] = ec_loss_avg
                total_loss = total_loss + self.diffusion.ec_weight * ec_loss_avg
            ...
            # Update EMA models
            if isinstance(self.diffusion, torch.nn.DataParallel):
                update_ema(self.ema_model.parameters(), self.diffusion.module._denoise_fn.parameters(), rate=self.ema_decay)
                update_ema(self.ema_num_schedule.parameters(), self.diffusion.module.num_schedule.parameters(), rate=self.ema_decay)
                update_ema(self.ema_cat_schedule.parameters(), self.diffusion.module.cat_schedule.parameters(), rate=self.ema_decay)
            else:
            ...
            if total_loss < best_loss and self.curr_epoch > 4000:
                ...
                if isinstance(self.diffusion, torch.nn.DataParallel):
                    state_dicts = {
                        'denoise_fn': self.diffusion.module._denoise_fn.state_dict(), 
                        'num_schedule': self.diffusion.module.num_schedule.state_dict(), 
                        'cat_schedule': self.diffusion.module.cat_schedule.state_dict(),
                    }
                else:
                    state_dicts = {
            ...
            ema_mloss, ema_gloss, ema_ec_loss = self.compute_loss()
            self.to_model(curr_model, curr_num_schedule, curr_cat_schedule)
            ema_total_loss = ema_mloss + ema_gloss
            if self.diffusion.enable_ec_validation:
                ema_total_loss += self.diffusion.ec_weight * ema_ec_loss
            ema_loss_dict = {
            ...
            if self.diffusion.enable_ec_validation:
                ema_loss_dict["ema_loss/ec_loss"] = ema_ec_loss
            ...
            if (epoch+1) % self.check_val_every == 0:
                ...
                out_metrics, _, _ = self.evaluate_generation(save_metric_details=True, plot_density=False)  # 禁用密度图避免容器环境问题
                ...
                ema_out_metrics, _, _ = self.evaluate_generation(ema=True, save_metric_details=True, plot_density=False)  # 禁用密度图避免容器环境问题
                log_dict.update({
                    "ema": ema_out_metrics,
                })
    ...
    def to_ema_model(self):
        if isinstance(self.diffusion, torch.nn.DataParallel):
            curr_model = self.diffusion.module._denoise_fn
            curr_num_schedule = self.diffusion.module.num_schedule
            curr_cat_schedule = self.diffusion.module.cat_schedule
            self.diffusion.module._denoise_fn = self.ema_model  # temporarily install the ema parameters into the model
            self.diffusion.module.num_schedule = self.ema_num_schedule
            self.diffusion.module.cat_schedule = self.ema_cat_schedule
        else:
    ...
    def to_model(self, curr_model, curr_num_schedule, curr_cat_schedule):
        if isinstance(self.diffusion, torch.nn.DataParallel):
            self.diffusion.module._denoise_fn = curr_model      # give back the parameters
            self.diffusion.module.num_schedule = curr_num_schedule
            self.diffusion.module.cat_schedule = curr_cat_schedule
        else:
    ...
    if info['task_type'] == 'regression':
        for i in range(len(num_col_idx) + len(cat_col_idx) + len(target_col_idx)):
            if i in set(num_col_idx):
                ...
                syn_df[i] = syn_cat[:, idx_mapping[i]]  # 直接使用idx_mapping，不减偏移
            else:
                syn_df[i] = syn_target[:, idx_mapping[i]]  # 直接使用idx_mapping，不减偏移
