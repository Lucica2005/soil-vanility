...
# Add import for differentiable EC calculator
...
try:
    ...
    DIFFERENTIABLE_EC_AVAILABLE = True
except ImportError:
    print("Warning: DifferentiableECCalculator not available. EC validation will use fallback.")
    DIFFERENTIABLE_EC_AVAILABLE = False

# Original EC converter import (fallback)
try:
    from lat_lon_to_ec_calculator import LatLonToECConverter
    EC_CALCULATOR_AVAILABLE = True
except ImportError:
    print("Warning: LatLonToECConverter not available. EC validation will be disabled.")
    EC_CALCULATOR_AVAILABLE = False
...
class UnifiedCtimeDiffusion(torch.nn.Module):
    def __init__(
        ...
        # EC validation parameters
        self.enable_ec_validation = enable_ec_validation and EC_CALCULATOR_AVAILABLE
        self.ec_weight = ec_weight
        self.lat_col_idx = lat_col_idx
        self.lon_col_idx = lon_col_idx
        self.ec_col_idx = ec_col_idx
        self.ec_is_target = ec_is_target
        
        if self.enable_ec_validation:
            if lat_col_idx is None or lon_col_idx is None or ec_col_idx is None:
                print("Warning: EC validation enabled but column indices not provided. Disabling EC validation.")
                self.enable_ec_validation = False
            else:
                try:
                    self.ec_converter = LatLonToECConverter()
                    if self.ec_is_target:
                        print("EC validation enabled with weight:", self.ec_weight, "(EC is TARGET column)")
                    else:
                        print("EC validation enabled with weight:", self.ec_weight, "(EC is FEATURE column)")
                except Exception as e:
                    print(f"Failed to initialize EC converter: {e}")
                    self.enable_ec_validation = False
    ...
    def mixed_loss(self, x):
        ...
        # Predict orignal data (distribution) - KEEP ORIGINAL WAY
        ...
        # Add EC validation loss if enabled - SIMPLIFIED APPROACH
        ec_loss = torch.zeros((1,)).float().to(self.device)
        if self.enable_ec_validation and x_num.shape[1] > 0:
            # Simply pass the full input data x to get access to target
            ec_loss = self._compute_ec_validation_loss_simple(x, model_out_num)
            
        return d_loss.mean(), c_loss.mean(), ec_loss.mean()
    ...
    def _compute_ec_validation_loss_simple(self, x, model_out_num):
        """
        Fixed EC validation loss that preserves gradient computation
        
        Args:
            x: Input tensor [batch_size, features] containing all data including target
            model_out_num: Model output for numerical features [batch_size, num_features]
            
        Returns:
            ec_loss: Differentiable EC validation loss tensor
        """
        try:
            # Debug: Print tensor shapes
            batch_size_x = x.shape[0]
            batch_size_model = model_out_num.shape[0]
            
            if batch_size_x != batch_size_model:
                print(f"Warning: Batch size mismatch - x: {batch_size_x}, model_out_num: {batch_size_model}")
                # Use the smaller batch size to avoid errors
                min_batch_size = min(batch_size_x, batch_size_model)
                x = x[:min_batch_size]
                model_out_num = model_out_num[:min_batch_size]
            
            # FIX 1: Vectorized coordinate extraction (NO .item() calls)
            lat_coords = x[:, self.lat_col_idx]  # Shape: [batch_size]
            lon_coords = x[:, self.lon_col_idx]  # Shape: [batch_size]
            
            # FIX 2: Vectorized predicted EC extraction
            predicted_ec = model_out_num[:, 0]  # First position is target (EC)
            
            # Check if we have valid coordinates
            if lat_coords.shape[0] == 0 or lon_coords.shape[0] == 0:
                return torch.tensor(0.0, device=self.device, requires_grad=True)
            
            # FIX 3: Use differentiable EC calculation
            true_ec = self._calculate_ec_differentiable(lat_coords, lon_coords)
            
            # Ensure predicted_ec and true_ec have the same shape
            if predicted_ec.shape != true_ec.shape:
                print(f"Shape mismatch: predicted_ec {predicted_ec.shape}, true_ec {true_ec.shape}")
                # Resize to match the smaller one
                min_size = min(predicted_ec.shape[0], true_ec.shape[0])
                predicted_ec = predicted_ec[:min_size]
                true_ec = true_ec[:min_size]
            
            # FIX 4: Vectorized loss calculation
            ec_loss = (predicted_ec - true_ec) ** 2
            
            # FIX 5: Handle invalid coordinates with torch.where
            valid_mask = self._get_valid_coordinates_mask(lat_coords[:ec_loss.shape[0]], lon_coords[:ec_loss.shape[0]])
            ec_loss = torch.where(valid_mask, ec_loss, torch.zeros_like(ec_loss))
            
            # Return mean loss (preserves gradients)
            return ec_loss.mean()
            
        except Exception as e:
            print(f"EC validation loss calculation error: {e}")
            # Print more debugging info
            if 'x' in locals():
                print(f"x shape: {x.shape}")
            if 'model_out_num' in locals():
                print(f"model_out_num shape: {model_out_num.shape}")
            if 'lat_coords' in locals():
                print(f"lat_coords shape: {lat_coords.shape}")
            if 'lon_coords' in locals():
                print(f"lon_coords shape: {lon_coords.shape}")
            if 'predicted_ec' in locals():
                print(f"predicted_ec shape: {predicted_ec.shape}")
            if 'true_ec' in locals():
                print(f"true_ec shape: {true_ec.shape}")
            
            # Return tensor with requires_grad=True to maintain gradient flow
            return torch.tensor(0.0, device=self.device, requires_grad=True)
    
    def _compute_ec_validation_loss(self, x_num, model_out_num):
        """
        Fixed EC validation loss for numerical features only
        
        Args:
            x_num: Numerical input features [batch_size, num_features]
            model_out_num: Model output for numerical features [batch_size, num_features]
        ...
        try:
            ...
            batch_size_x = x_num.shape[0]
            batch_size_model = model_out_num.shape[0]
            
            if batch_size_x != batch_size_model:
                print(f"Warning: Batch size mismatch - x_num: {batch_size_x}, model_out_num: {batch_size_model}")
                # Use the smaller batch size to avoid errors
                min_batch_size = min(batch_size_x, batch_size_model)
                x_num = x_num[:min_batch_size]
                model_out_num = model_out_num[:min_batch_size]
            
            # FIX 1: Vectorized coordinate extraction (NO .item() calls)
            lat_coords = x_num[:, self.lat_col_idx]  # Shape: [batch_size]
            lon_coords = x_num[:, self.lon_col_idx]  # Shape: [batch_size]
            
            # FIX 2: Vectorized predicted EC extraction
            if self.ec_is_target:
                # EC is the target - get from model output
                predicted_ec = model_out_num[:, 0] if model_out_num.shape[1] > 0 else model_out_num.squeeze()
            else:
                # EC is a feature
                predicted_ec = model_out_num[:, self.ec_col_idx]
            ...
            print(f"Error in EC validation loss computation: {e}")
    ...
    def _calculate_ec_differentiable(self, lat_coords, lon_coords):
        """
        Calculate EC values using differentiable operations
        ...
        """
        if not hasattr(self, 'differentiable_ec_calculator'):
            # Initialize differentiable EC calculator on first use
            if DIFFERENTIABLE_EC_AVAILABLE:
                # Try to load RGB array from world_map_rgb.npy
                rgb_path = os.path.join(os.path.dirname(__file__), '..', 'world_map_rgb.npy')
                if not os.path.exists(rgb_path):
                    # Fallback to relative path
                    rgb_path = 'tabdiff/world_map_rgb.npy'
                
                self.differentiable_ec_calculator = DifferentiableECCalculator(
                    rgb_array_path=rgb_path,
                    device=self.device
                )
                print("Initialized DifferentiableECCalculator")
            else:
                # Fallback: Use simple approximation
                print("Using fallback EC calculation (simple approximation)")
                self.differentiable_ec_calculator = None
        
        if self.differentiable_ec_calculator is not None:
            return self.differentiable_ec_calculator.calculate_ec_from_coordinates_differentiable(
                lat_coords, lon_coords
            )
        else:
            # Fallback: Simple linear approximation (differentiable)
            # This ensures gradients still flow even without proper EC calculation
            return lat_coords * 0.1 + lon_coords * 0.2
    
    def _get_valid_coordinates_mask(self, lat_coords, lon_coords):
        """
        Create mask for valid coordinates using tensor operations
