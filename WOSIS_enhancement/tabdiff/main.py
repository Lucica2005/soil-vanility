...
def main(args):
    ...
    if args.multi_gpu and torch.cuda.device_count() > 1:
        ...
        print(f"All device IDs: {device_ids}")
    else:
    ...
    metrics = TabMetrics(real_data_path, test_data_path, val_data_path, info, primary_device, metric_list=metric_list)
    ...
    if args.impute:
        ...
        if y_only_model_path is None:
            ...
            assert y_only_model_path_arr, f"Cannot not infer y_only model's ckpt_path from {y_only_model_parent_path}, please make sure that you first train a y_only model before training the y_only model!"
        ...
        y_only_model.to(primary_device)
        # load weights
        state_dicts = torch.load(y_only_model_path, map_location=primary_device)
    ...
    # Add EC validation parameters if available
    ec_params = {}
    if hasattr(args, 'enable_ec_validation'):
        ec_params = {
            'enable_ec_validation': args.enable_ec_validation,
            'ec_weight': getattr(args, 'ec_weight', 0.1),
            'lat_col_idx': getattr(args, 'lat_col_idx', None),
            'lon_col_idx': getattr(args, 'lon_col_idx', None),
            'ec_col_idx': getattr(args, 'ec_col_idx', None),
            'ec_is_target': getattr(args, 'ec_is_target', False),
        }
    ...
    diffusion.to(primary_device)
    
    # Apply DataParallel for multi-GPU training
    if args.multi_gpu and torch.cuda.device_count() > 1:
        diffusion = torch.nn.DataParallel(diffusion, device_ids=device_ids)
